...
2. Home — **3 puntos de entrada obligatorios**:
...
3. `sitemap.xml`: `<url>` con `lastmod` del día + `image:image` del hero.
4. Componente "Otros proyectos": añadir entrada al array `ALL` (está duplicado por página; al menos en la página nueva y el resto cuando se toquen).
5. Verificar en preview (desktop + 390px móvil, sin scroll horizontal) y deploy.
6. Pedir indexación en Search Console.
